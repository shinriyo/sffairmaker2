...
def testCharSff():
    ...
    assert not sff.hasChanged() #キャラSFFへの設定は何も変化させない
    ...
    #char用SFFのときは、(0, 0), (9000, 0)のUseActを変更しようとしても無視
    ...
    #group, indexを変更すれば、「UseAct変更不可能」の効力は無し
...
def testOpenMissingFile():
    ...
    for i, name in enumerate(["air", "sff", "act"], start=1):
        ...
        with open(filename, "w") as fp:
            fp.write("くぁｗせｄｒｆｔｇｙふじこｌｐ")
        
        x.open(filename) #バグったファイルも、とりあえず読み込む
